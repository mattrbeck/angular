...
describe('ShadowCss', () => {
  ...
  it('should handle pseudo functions correctly', () => {
    ...
    expect(shim(':where(.one :host) {}', 'contenta', 'hosta')).toEqualCss(':where(.one .hosta) {}');
    expect(shim('div :where(.one) {}', 'contenta', 'hosta')).toEqualCss(
    ...
    expect(shim(':where(:is(a)) {}', 'contenta', 'hosta')).toEqualCss(':where(:is(a.contenta)) {}');
    ...
    ).toEqualCss(':where(a.contenta, b.contenta:where(.foo), .contenta:has(.bar):first-child) {}');
